...
const QBOX_SETTING_KEYS = [
  ...
  'qbox_money_col',
  'qbox_job_col',
  'qbox_person_custom_fields',
  'qbox_vehicle_custom_fields',
];
...
export default function AdminFieldMappings() {
  ...
  const [savingQboxSettings, setSavingQboxSettings] = useState(false);
  const [testingQboxConnection, setTestingQboxConnection] = useState(false);
  const [qboxTestResult, setQboxTestResult] = useState(null);
  const [qboxSchemaResult, setQboxSchemaResult] = useState(null);
  ...
  function buildQboxSettingsPayload() {
    const payload = {};
    for (const key of QBOX_SETTING_KEYS) {
  ...
  async function saveQboxSettings(showAlert = true) {
    setSavingQboxSettings(true);
    try {
      const payload = buildQboxSettingsPayload();
      await api.put('/api/admin/settings', { settings: payload });
      if (showAlert) {
        alert('QBox settings saved');
      }
      return true;
    } catch (err) {
      ...
      setSavingQboxSettings(false);
    }
  }

  async function testQboxConnection() {
    setTestingQboxConnection(true);
    setQboxTestResult(null);
    setQboxSchemaResult(null);
    try {
      const payload = buildQboxSettingsPayload();
      await api.put('/api/admin/settings', { settings: payload });
      ...
      setQboxSchemaResult(schema);
    } catch (err) {
      setQboxTestResult({ success: false, message: formatErr(err) });
      if (err.details) {
        setQboxSchemaResult(err.details);
      }
    } finally {
      setTestingQboxConnection(false);
    }
  }
      ...
      <div className="bg-cad-card border border-cad-border rounded-lg p-5 mb-4">
        ...
        <div className="mt-3 space-y-3">
          <div>
            ...
            <textarea
              ...
              placeholder={'[{"key":"job","source":"charinfo","path":"job.label"}]'}
            />
          </div>
          <div>
            ...
            <textarea
              ...
              className="w-full h-24 bg-cad-surface border border-cad-border rounded px-3 py-2 text-xs font-mono focus:outline-none focus:border-cad-accent"
              placeholder={'[{"key":"mods_class","source":"column","column":"mods","path":"class"}]'}
            />
          </div>
          <p className="text-xs text-cad-muted">
            Legacy JSON field arrays are still supported. Advanced field-by-field mapping is configured below.
          </p>
        </div>

        <div className="flex flex-wrap items-center gap-2 mt-4">
          <button
            onClick={() => saveQboxSettings(true)}
            disabled={savingQboxSettings}
          ...
          >
            {savingQboxSettings ? 'Saving...' : 'Save QBox Settings'}
          </button>
          <button
            onClick={testQboxConnection}
            disabled={testingQboxConnection}
          ...
          >
            {testingQboxConnection ? 'Testing...' : 'Test Connection'}
          </button>
          {qboxTestResult && (
            <span className={`text-sm whitespace-pre-wrap ${qboxTestResult.success ? 'text-emerald-400' : 'text-red-400'}`}>
              {qboxTestResult.message}
            </span>
          )}
        </div>
        ...
        {qboxSchemaResult && (
          <div className="mt-3 text-xs">
            {Array.isArray(qboxSchemaResult.errors) && qboxSchemaResult.errors.length > 0 && (
              <div className="text-red-400 whitespace-pre-wrap">
                {'Errors:\n- ' + qboxSchemaResult.errors.join('\n- ')}
              </div>
            )}
            {Array.isArray(qboxSchemaResult.players?.warnings) && qboxSchemaResult.players.warnings.length > 0 && (
              <div className="text-amber-300 whitespace-pre-wrap mt-2">
                {'Player Warnings:\n- ' + qboxSchemaResult.players.warnings.join('\n- ')}
              </div>
            )}
            {Array.isArray(qboxSchemaResult.vehicles?.warnings) && qboxSchemaResult.vehicles.warnings.length > 0 && (
              <div className="text-amber-300 whitespace-pre-wrap mt-2">
                {'Vehicle Warnings:\n- ' + qboxSchemaResult.vehicles.warnings.join('\n- ')}
              </div>
            )}
            {qboxSchemaResult.success && (
              <div className="text-emerald-400 mt-2">
                Schema check passed. Charinfo JSON column: {String(qboxSchemaResult.players?.columns?.find(c => c.name === (settings.qbox_charinfo_col || 'charinfo'))?.isJson || false)}
              </div>
            )}
          </div>
        )}
