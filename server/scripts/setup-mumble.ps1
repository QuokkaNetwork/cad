...
# Config
# ---------------------------------------------------------------------------
$MumbleVersion  = '1.4.287'
$DownloadUrl    = "https://github.com/mumble-voip/mumble/releases/download/v$MumbleVersion/MumbleServer-$MumbleVersion.Windows.x64.zip"
$TargetDir      = Join-Path $PSScriptRoot '..\murmur'
$TargetExe      = Join-Path $TargetDir 'murmur.exe'
$TempZip        = Join-Path $env:TEMP 'murmur-download.zip'
$TempExtract    = Join-Path $env:TEMP 'murmur-extract'
$MumblePort     = 64738
...
if (Test-Path $TargetExe) {
    ...
    Write-Host "[1/3] Downloading Mumble Server v$MumbleVersion ..." -ForegroundColor Cyan
    ...
    try {
        ...
        Write-Host '  2. Download the Windows Mumble Server (murmur) zip'
        Write-Host "  3. Extract murmur.exe into: $TargetDir"
        exit 1
    }

    Write-Host "[2/3] Extracting murmur.exe ..." -ForegroundColor Cyan

    # Extract zip
    if (Test-Path $TempExtract) { Remove-Item $TempExtract -Recurse -Force }
    Expand-Archive -Path $TempZip -DestinationPath $TempExtract -Force

    # Find murmur.exe anywhere in the extracted tree
    $Found = Get-ChildItem -Path $TempExtract -Filter 'murmur.exe' -Recurse | Select-Object -First 1

    if (-not $Found) {
        # Some releases name it mumble-server.exe
        $Found = Get-ChildItem -Path $TempExtract -Filter 'mumble-server.exe' -Recurse | Select-Object -First 1
    }

    if (-not $Found) {
        Write-Host '[ERROR] Could not locate murmur.exe or mumble-server.exe in the downloaded zip.' -ForegroundColor Red
        Write-Host "        Zip contents in $TempExtract — please extract manually."
        exit 1
    }
...
# ---------------------------------------------------------------------------
Write-Host ''
Write-Host '[3/3] Configuring Windows Firewall (port 64738 TCP + UDP) ...' -ForegroundColor Cyan
...
Write-Host "  1. Open port $MumblePort TCP+UDP in your VPS provider's security group / network firewall panel"
Write-Host '  2. Add the voice convars to your FiveM server.cfg (see voice.cfg in the repo root)'
Write-Host '  3. Ensure MUMBLE_MANAGE=true and MUMBLE_HOST=127.0.0.1 are set in server/.env'
Write-Host '  4. Restart the CAD server — it will start Murmur automatically'
Write-Host '  5. Restart the FiveM server so pma-voice picks up the new convars'
Write-Host ''
