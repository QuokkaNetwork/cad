 ...
 * We read it, substitute the IP/port values from .env, then write
...
function getVoiceCfgConfig() {
  ...
  return { serverPath, publicIp, port };
}

function buildVoiceCfgContent(publicIp, port) {
  if (!fs.existsSync(VOICE_CFG_TEMPLATE)) {
    return generateVoiceCfgInline(publicIp, port);
  }
...
function generateVoiceCfgInline(publicIp, port) {
  return [
...
function deployVoiceCfg() {
  ...
  try {
    ...
    const content = buildVoiceCfgContent(publicIp || '0.0.0.0', port);
